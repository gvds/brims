    ...
    <div class="h-[calc(100vh-14rem)] flex flex-col w-full">
        ...
        <div>
            ...
            <div class="flex gap-5 px-5 py-2 rounded-md max-w-max bg-white border-zinc-200 dark:bg-zinc-900 border dark:border-zinc-800 shadow-sm font-semibold">
                @foreach ($projects as $index => $project)
                    <div style="color: color-mix(in srgb, {{ $this->colours[$project->id] }} 80%, light-dark(black, white)">{{ $project->title }}</div>
                @endforeach
            </div>
        </div>
        ...
        <div class="flex justify-between">
            ...
            <div>
                ...
                <span class="border border-zinc-300 dark:border-zinc-700 rounded-md shadow-.md bg-zinc-200 dark:bg-zinc-800 px-3 py-1 cursor-pointer" wire:click='nextmonth'>Next</span>
            </div>
        </div>

        <div class="border rounded-md border-zinc-300 dark:border-zinc-700! bg-zinc-100 dark:bg-zinc-900! shadow-md w-full h-full flex flex-col overflow-hidden">
            ...
            <div class="grid grid-cols-7 {{ 'grid-rows-' . count($weeks) }} border-t border-l border-zinc-200 dark:border-zinc-700! text-xs grow h-full auto-rows:1fr">
