...
class PDF_Code128 extends FPDF
{
	...
	function Code128($x, $y, $code, $w, $h)
	{
		...
		for ($i = 0; $i < strlen((string) $code); $i++) {
			$needle = substr((string) $code, $i, 1);
			$Aguid .= ((!str_contains((string) $this->Aset, $needle)) ? "N" : "O");
			$Bguid .= ((!str_contains((string) $this->Bset, $needle)) ? "N" : "O");
			$Cguid .= ((!str_contains((string) $this->Cset, $needle)) ? "N" : "O");
		}
		...
		while ($code > "") {
			...
			if (Str::startsWith($Cguid, $SminiC)) {                                  // jeu C
				...
				for ($i = 0; $i < $made; $i += 2) {
					$crypt .= chr(strval(substr((string) $code, $i, 2)));                          // conversion 2 par 2
				}
				$jeu = "C";
			} else {
				...
				$crypt .= strtr(substr((string) $code, 0, $made), $this->SetFrom[$jeu], $this->SetTo[$jeu]); // conversion selon jeu

			}
			$code = substr((string) $code, $made);                                           // raccourcir légende et guides de la zone traitée
